...
unsigned int SendPort = 12347;		// Arduino send udp message to this Port(that of mac).
...
void StateChart(){
	switch(State){
		case STATE__WAIT_COMMAND:
			if(str_Command == "set_Angle"){
				operation__SetAngle(udp_Param[0]);
			}
			break;
	}
}
...
void send_udp_Beat_to_oF(){
	Udp.beginPacket(IP_SendTo, SendPort);
		String str_for_Send = "PrjShutter[p]Beat";
